	...
	class Page
		class << self
			...
			def get(name)
				...
				if File.exist? path
					...
					if File.exist?(default_path)
						...
						put name, {'title'=>name,'story'=>[{'type'=>'factory', 'id'=>Sfw::RandomId.generate}]} unless File.file? path
					end
				end
			end
			...
			def assert_directories_set
				raise PageError.new('Sfw::Page.directory must be set') unless directory
				raise PageError.new('Sfw::Page.default_directory must be set') unless default_directory
			end
		end
	end
end
